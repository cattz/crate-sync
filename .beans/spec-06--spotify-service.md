...
An `updatePlaylistDescription(playlistId, description)` method enables pushing serialized tags and notes back to the Spotify playlist description field, supporting the bidirectional description sync workflow orchestrated by spec-08.
...
class SpotifyService {
  ...
  async updatePlaylistDescription(playlistId: string, description: string): Promise<void>
...
| PUT | `/playlists/{id}` | `renamePlaylist()` (body: `{ name }`), `updatePlaylistDetails()` (body: `{ name?, description? }`), `updatePlaylistDescription()` (body: `{ description }`) |
...
Thin wrapper around `updatePlaylistDetails()`. Calls:
```ts
async updatePlaylistDescription(playlistId: string, description: string): Promise<void> {
  await this.updatePlaylistDetails(playlistId, { description });
}
```
This is a convenience method for the push workflow (spec-08) which composes a description from tags+notes and pushes it to Spotify.
...
| updatePlaylistDescription with empty string | Sends empty description to Spotify (valid; clears description) |
...
- Verify `updatePlaylistDescription()` calls `updatePlaylistDetails()` with `{ description }`.
...
- updatePlaylistDescription pushes description string to Spotify API
...
- [ ] `updatePlaylistDescription()` pushes description string via `updatePlaylistDetails()`
...
- [ ] Unit tests with mocked fetch covering pagination, token refresh, batching, shutdown, description push
